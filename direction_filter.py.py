...
class KalmanTrend2D:
    ...
    def filter_series(self, y: np.ndarray):
        ...
        self.x = np.array([y[0], 0.0]) if n > 0 else np.array([0.0, 0.0])
        ...
        for i, z in enumerate(y):
            ...
            S = H @ self.P @ H.T + R
            K = (self.P @ H.T) / S
            self.x = self.x + (K.flatten() * (z - z_pred.item()))
            self.P = (I - K @ H) @ self.P
            
            price_est[i] = self.x[0]
            slope_est[i] = self.x[1] # <-- ИСПРАВЛЕНИЕ ЗДЕСЬ (было self.x[4])
...
class BTCDirectionFilter:
    ...
    def run(self, df: pd.DataFrame, hmm_trend_prob_series: pd.Series) -> pd.DataFrame:
        ...
        score = (self.cfg.w_k * slope_z) + (self.cfg.w_st * st_dir.values) + (self.cfg.w_hmm * (2.0 * hmm_prob - 1.0))
